...
def risk_msv_122(snr,gamma):
  return 1+snr*(1-np.sqrt(2*gamma))**2/gamma

def risk_msv(snr,gamma):
  return risk0_01(snr,gamma)*(gamma<=0.5)+risk_msv_122(snr,gamma)*((gamma>0.5)&(gamma<2))+risk0_1inf(snr,gamma)*(gamma>=2)


fig, ax=plt.subplots(1,2,figsize=(6,2.8),gridspec_kw={'width_ratios': [2.5,1]})

gammas=np.geomspace(1e-1,1e1,1000)
for c, snr in enumerate([1,2,5,10]):
  _=ax[0].plot(gammas, risk_msv(snr,gammas),'C'+str(c))
...
_=ax[0].set_title('$\\overline{R_X}(\\lambda,\\gamma,\\text{SNR})/\\sigma_\\varepsilon^2$')
...
max_qs=[]
for snr in snrs:
  max_qs.append(np.max(risk_msv_122(snr,gammas2)/opt_risk(snr,gammas2)))
...
_=ax[1].set_title('$\\max_\\gamma \\ \\overline{R_X}(\\overline{\\lambda_T})/\\overline{R_X}(\\overline{\\lambda^*})$')
